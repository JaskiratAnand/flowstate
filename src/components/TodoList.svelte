  ...
  <div class="space-y-4">
    <div class="relative">
      <input
        ...
        class="w-full bg-surface shadow-[var(--shadow-pressed)] rounded-2xl px-6 py-4.5 text-sm font-semibold text-text-primary outline-none transition-all placeholder:text-text-tertiary placeholder:font-normal"
      />
      <button
        on:click={addTask}
        class="absolute right-3 top-1/2 -translate-y-1/2 bg-surface shadow-[var(--shadow-ambient)] text-accent w-10 h-10 rounded-xl flex items-center justify-center active:shadow-[var(--shadow-pressed)] active:scale-95 transition-all"
        aria-label="Add task"
      >
  ...
  >
    ...
    {:else}
      <div class="flex flex-col items-center justify-center py-20 text-center opacity-40">
        <svg class="w-16 h-16 mb-4 text-text-tertiary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
          <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/>
        </svg>
        <p class="text-sm font-medium text-text-tertiary">No focus sessions yet today.<br/>Plant a seed.</p>
      </div>
    {/each}
  </div>
</div>
